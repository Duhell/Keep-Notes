...
@section('content')
    <div class="relative w-full h-[100vh] md:flex md:justify-center md:items-center md:flex-col bg-cyan-600">
        <p class="mb-5 text-[1.5rem] font-medium text-[#ebebeb]">Edit Note</p>
        <div class="bg-slate-200 p-5 rounded w-[700px] relative">
            ...
            <form action="{{ url('edit') }}" method="post" class="mt-3">
                @csrf
                <input type="hidden" value="{{ $oldData->id }}" name="noteid">
                <div class="md:flex md:flex-col relative mb-2">
                    <input type="text" value="{{ $oldData->title }}" name="title" id="title" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 appearance-none focus:outline-none focus:ring-1 focus:border-blue-600 peer" placeholder=" " />
                ...
                <div class="md:flex md:flex-col relative mb-2">
                    <textarea type="text"  name="note" id="note" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 appearance-none focus:outline-none focus:ring-1 focus:border-blue-600 peer" placeholder=" "> {{ $oldData->notes }}</textarea>
                ...
                <div class="w-full h-full md:flex justify-center mt-4">
                    <button type="submit" class="text-white w-full bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">Submit</button>
                </div>
                <div class="pr-2 mt-3">
